...
class CheckInController extends Controller
{
    ...
    public function show($lichKhoiHanhId, $chiTietId)
    {
        ...
        $datTours = DatTour::with([ 'nguoiDung', 'khachHangs'])
            ->where('lich_khoi_hanh_id', $lichKhoiHanhId)
            ->get();

        $chiTiet = $chiTietObj;
        $checkedIds = CheckInKhachHang::where('chi_tiet_lich_trinh_id',$chiTietId)
            ->whereIn('trang_thai', [
                'da_check_in',
                'da_check_out'
            ])
        ...
        $checkIns = CheckInKhachHang::where('chi_tiet_lich_trinh_id', $chiTietId)
            ->get()
            ->keyBy('khach_hang_dat_tour_id');

        $tongKhach = 0;
        foreach ($datTours as $datTour) { $tongKhach += $datTour->khachHangs->count(); }
        ...
        [$checkinWindowStart, $checkinWindowEnd] = $this->getCheckinWindow($lichKhoiHanh, $chiTietObj);
    ...
    */
    ...
    public function checkOutBu(Request $request, $id)
    {
        ...
        // Phải check-in rồi mới được check-out bù
        if ($checkIn->trang_thai !== 'da_check_in') {
            return back()->with(
                'error',
                'Khách chưa check-in hoặc đã check-out.'
            );
        }
        ...
        // Check-out bù chỉ dùng khi hoạt động đã hết thời gian
        if (!$this->isCheckinWindowExpired($lichKhoiHanh, $chiTiet)) {
            return back()->with(
                'error',
        ...
        // Kiểm tra HDV có được phân công tour không
        $assigned =
            (
                $lichKhoiHanh->huong_dan_vien_id &&
                (int) $lichKhoiHanh->huong_dan_vien_id === (int) $guide->id
        ...
        // Nếu DB của bạn ĐÃ CÓ các cột này thì mở ra:
        /*
    $checkIn->is_checkout_bu = true;
    $checkIn->ly_do_checkout_bu =
        trim($request->ly_do_checkout_bu);
    $checkIn->thoi_gian_ghi_nhan_checkout_bu = now();
    */
    ...
    public function finishTour(Request $request, LichKhoiHanhTour $lichKhoiHanh)
    {
        ...
        CheckinSave::updateOrCreate(
            ['lich_khoi_hanh_id' => $lichKhoiHanh->id,'action' => 'CONFIRM_KET_THUC_TOUR',],
            ['huong_dan_vien_id' => $guide->id,]
        );
