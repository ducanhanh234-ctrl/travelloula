...
<div class="guide-form-page">
    ...
    <form action="{{ route('Admin.huong-dan-viens.update', $huongDanVien->id) }}" method="POST" enctype="multipart/form-data" class="guide-form-card">
        @csrf
        @method('PUT')
        ...
        <div class="row g-4">
            <div class="col-md-6">
                ...
                <input type="text" name="ho_ten" class="form-control custom-input" value="{{ old('ho_ten', $huongDanVien->ho_ten) }}">
                <div class="text-danger small mt-1">{{ $errors->first('ho_ten') }}</div>
            </div>

            <div class="col-md-6">
                <label class="form-label">Email</label>
                <input type="email" name="email" class="form-control custom-input" value="{{ old('email', $huongDanVien->email) }}">
                <div class="text-danger small mt-1">{{ $errors->first('email') }}</div>
            </div>

            <div class="col-md-6">
                <label class="form-label">Mật khẩu mới</label>
                <input type="password" name="password" class="form-control custom-input" placeholder="Để trống nếu không đổi">
                <div class="text-danger small mt-1">{{ $errors->first('password') }}</div>
            </div>

            <div class="col-md-6">
                <label class="form-label">Số điện thoại</label>
                <input type="text" name="so_dien_thoai" class="form-control custom-input" value="{{ old('so_dien_thoai', $huongDanVien->so_dien_thoai) }}">
            </div>

            <div class="col-md-6">
                <label class="form-label">Ngày sinh</label>
                <input type="date" name="ngay_sinh" class="form-control custom-input" value="{{ old('ngay_sinh', optional($huongDanVien->ngay_sinh)->format('Y-m-d')) }}">
            </div>
            ...
            <div class="col-md-12">
                ...
                <input type="text" name="dia_chi" class="form-control custom-input" value="{{ old('dia_chi', $huongDanVien->dia_chi) }}">
            </div>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="so_nam_kinh_nghiem" class="form-control custom-input" value="{{ old('so_nam_kinh_nghiem', $huongDanVien->so_nam_kinh_nghiem) }}" min="0">
            </div>
