...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const lichKhoiHanhId = document.querySelector('input[name="lich_khoi_hanh_id"]').value;
        ...
        // Gọi API lấy danh sách xe không bị trùng lịch
        async function loadAvailableVehicles() {
            try {
                const response = await fetch(`{{ route('phan-cong.api.available-vehicles') }}?lich_khoi_hanh_id=${lichKhoiHanhId}`);
        ...
        // Gọi API lấy danh sách HDV không bị trùng lịch
        async function loadAvailableGuides() {
            try {
                const response = await fetch(`{{ route('phan-cong.api.available-guides') }}?lich_khoi_hanh_id=${lichKhoiHanhId}`);
                const data = await response.json();
...
if (selectedGuideIds.length >= requiredGuides) {
    ...
    guideCheckboxes.forEach(cb => {
        ...
        function setupFilter(searchEl, dropdownEl, items) {
            searchEl.addEventListener('input', function() {
                toggleDropdown(searchEl, dropdownEl, true);
                const filter = this.value.toLowerCase();
                const checkboxes = dropdownEl.querySelectorAll('.custom-dropdown-item');
        ...
        setupFilter(vehicleSearch, vehicleDropdown, availableVehicles);
        setupFilter(guideSearch, guideDropdown, availableGuides);
