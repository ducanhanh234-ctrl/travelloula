            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('Guide.baocaosuco.store') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input name="tieu_de" value="{{ old('tieu_de') }}"
                            class="form-control @error('tieu_de') is-invalid @enderror" required>
                        @error('tieu_de')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="loai_su_co" class="form-select" required>
                                ...
                                @foreach (\App\Models\BaoCaoSuCo::loaiSuCoList() as $value => $label)
                                    <option value="{{ $value }}" @selected(old('loai_su_co') === $value)>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="muc_do" class="form-select" required>
                                @foreach (\App\Models\BaoCaoSuCo::mucDoList() as $value => $label)
                                    <option value="{{ $value }}" @selected(old('muc_do', 'trung_binh') === $value)>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
