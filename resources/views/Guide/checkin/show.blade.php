    ...
    <div class="checkin-page fade-in">
        ...
        <div class="checkin-progress-card">
            ...
            <div class="checkin-progress-track">
                <div class="checkin-progress-bar" role="progressbar" style="width: {{ $phanTramCheckIn }}%;"
                    aria-valuenow="{{ $phanTramCheckIn }}" aria-valuemin="0" aria-valuemax="100">
        ...
        <div class="checkin-main-actions">
            <form action="{{ route('Guide.checkin.checkinTatCa') }}" method="POST"
            ...
            <form action="{{ route('Guide.checkin.undoTatCa') }}" method="POST"
                onsubmit="return confirm('Bạn có chắc muốn hoàn tác tất cả trạng thái Check-in và Check-out của hành khách?');">
                @csrf
            ...
            <div class="checkin-table-wrapper">
                <table class="table checkin-table">
                    ...
                    <tbody>
                        ...
                        @if ($tongKhach > 0)
                            @foreach ($datTours as $datTour)
                                @foreach ($datTour->khachHangs as $khach)
                                    ...
                                    <tr>
                                        <td><span class="checkin-number">{{ $stt++ }}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            @if ($checkIn && ($checkIn->is_checkin_bu ?? false))
                                                <div class="checkin-note checkin-makeup-note">
                                                    ...
                                                    @if ($checkIn->thoi_gian_ghi_nhan_bu)
                                                        <div class="checkin-makeup-time">
                                                            Ghi nhận:
                                                            {{ $checkIn->thoi_gian_ghi_nhan_bu->format('H:i d/m/Y') }}
                                                        </div>
                                                    @endif
                                                </div>
                                        ...
                                        <td>
                                                ...
                                                <div class="checkin-row-actions">
                                                    @if (!$checkIn || $checkIn->trang_thai === 'chua_check_in')
                                                        <button type="button"
                                                            ...
                                                            data-bs-target="#checkinBuModal{{ $khach->id }}"
                                                            title="Hoạt động đã kết thúc - thực hiện Check-in bù">
                                            ...
                                            @elseif (!$checkIn || $checkIn->trang_thai === 'chua_check_in')
                                                <div class="checkin-row-actions">
                                                    ...
                                                    @endif
                                                    <button type="button" class="btn-checkin-row btn-row-note" data-bs-toggle="modal"
                                            ...
                                            @elseif ($checkIn->trang_thai === 'da_check_in')
                                                <div class="checkin-row-actions">
                                                    ...
                                                    <form action="{{ route('Guide.checkin.undo', $checkIn->id) }}" method="POST">
                                                        @csrf
                                                        <button type="submit" class="btn-checkin-row btn-row-undo" title="Hoàn tác Check-in"
                                            ...
                                            @elseif ($checkIn->trang_thai === 'da_check_out')
                                                <div class="checkin-row-actions">
                                                    <form action="{{ route('Guide.checkout.undo', $checkIn->id) }}" method="POST">
                                                        @csrf
                                                        <button type="submit" class="btn-checkin-row btn-row-checkout-undo"
                        ...
                        @else
                            <tr>
                                <td colspan="6" class="checkin-empty-row">
                                    <div class="checkin-empty-icon">
                                        <i class="fas fa-users-slash"></i>
                                    </div>

                                    <div class="checkin-empty-title">Chưa có hành khách</div>
...
@if($allCheckedOut)
    <div class="mt-4 d-flex justify-content-end">
        <form id="confirmCheckinForm"
              action="{{ route('Guide.checkin.saveLock', $lichKhoiHanhId) }}"
              method="POST">
            @csrf

            <input type="hidden" name="chi_tiet_id" value="{{ $chiTiet->id }}">
            <input type="hidden" name="ngay_thu" value="{{ $chiTiet->lichTrinh->ngay_thu }}">
            <input type="hidden" name="action" value="CONFIRM_CHI_TIET">
...
@endif
    ...
    @foreach ($datTours as $datTour)
        @foreach ($datTour->khachHangs as $khach)
            ...
            <div class="modal fade checkin-modal" id="ghiChuModal{{ $khach->id }}" tabindex="-1"
                aria-labelledby="ghiChuModalLabel{{ $khach->id }}" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <form action="{{ route('Guide.checkin.note') }}" method="POST">
                            @csrf
                            <div class="modal-header">
                                <h5 class="modal-title" id="ghiChuModalLabel{{ $khach->id }}">
                            ...
                            <div class="modal-body">
                                ...
                                <textarea name="ghi_chu" id="ghi_chu_{{ $khach->id }}" class="form-control" rows="4"
                                    ...
                                    required>{{ $checkIn->ghi_chu ?? '' }}</textarea>
                            </div>
            ...
            @if (($checkinExpired ?? false) && (!$checkIn || $checkIn->trang_thai === 'chua_check_in'))
                <div class="modal fade checkin-modal" id="checkinBuModal{{ $khach->id }}" tabindex="-1"
                    aria-labelledby="checkinBuModalLabel{{ $khach->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <form action="{{ route('Guide.checkin.check-in-bu') }}" method="POST">
                                @csrf

                                <div class="modal-header">
                                    <h5 class="modal-title" id="checkinBuModalLabel{{ $khach->id }}">
                                        <i class="fas fa-history"></i>
                                        Check-in bù
                                    </h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        <label for="ly_do_checkin_bu_{{ $khach->id }}" class="form-label fw-semibold">
                                            Lý do Check-in bù
                                            <span class="text-danger">*</span>
                                        </label>

                                        <textarea name="ly_do_checkin_bu"
                                            id="ly_do_checkin_bu_{{ $khach->id }}"
                                            class="form-control"
                                            rows="4"
                                            maxlength="500"
                                            placeholder="Ví dụ: Mất kết nối mạng tại địa điểm nên chưa thể thực hiện Check-in..."
                                            required></textarea>

                                        <div class="form-text">
                                            Vui lòng ghi rõ lý do thực hiện điểm danh sau thời gian hoạt động.
                                        </div>
                                    </div>
                                </div>
